import {defineClassMocker, mockData, mockInstance} from '.';
...
export function basicMockers(t: typeof basicTypes): typeof basicTypes {
    return {
        number: t.number.setMocker('basic', (type, {indexer}) => {
            let value = indexer();
        ...
        string: t.string.setMocker('basic', (type, {indexer}) => {
        ...
        boolean: t.boolean.setMocker('basic', (type, {indexer}) => indexer() % 2 === 0),

        bigint: t.bigint.setMocker('basic', (type, {indexer}) => BigInt(indexer())),

        symbol: t.symbol.setMocker('basic', () => {throw new Error(`It's impossible to mock Symbols`)}),

        array: t.array.setMocker('basic', (type, {indexer, strategy}) => {
        ...
        tuple: t.tuple.setMocker('basic', (type, {indexer, strategy}) => {
            indexer();
